...
import { POINTS_TO_FEFILL } from "@/constant";
...
export const Items = ({ hearts, points, hasActiveSubscription }: Props) => {
    ...
    const onRefillHearts = () => {
        if (pending || hearts === 5 || points < POINTS_TO_FEFILL) {
            return;
        }
    ...
    return (
        <ul className=" w-full">
            <div className=" flex items-center w-full p-4 gap-x-4 border-t-2">
                ...
                <Button disabled={pending || hearts === 5 || points < POINTS_TO_FEFILL} onClick={onRefillHearts}>
                    {hearts === 5 ? (
                        "full"
                    ) : (
                        <div className=" flex items-center">
                            <Image src="/points.svg" alt="Point" height={20} width={20} />
                            <p>{POINTS_TO_FEFILL}</p>
                        </div>
                    )}
                </Button>
            </div>
